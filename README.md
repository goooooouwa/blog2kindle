...
**Not to be confused of slice-[0-9].xml, which are actual files with RSS feeds content, where as slice-[0-9].txt consists only links to slices-[0-9].xml files**. slice-[0-9].txt is equivalent to feeds.txt, output of blog_crawler's "render" command, like this [feeds.txt](https://github.com/goooooouwa/rss-feeds/tree/master/codinghorror).
